...
public class grafico1 extends HttpServlet {
 Negocio   obj=new Negocio();
  JFreeChart circulo(int an){
   ...
   int cuenta=1;
   for(Articulo  x:obj.lisGenArt()){
       ds.setValue(x.getDes(), x.getPrecio());
       cuenta++;
       if(cuenta>an)break;
   }
  //graficar en memoria
  JFreeChart fg=ChartFactory.createPieChart3D("nro de articulos "+an, ds, true, true, true);
  return fg;  
}
//grafico de barras
JFreeChart barra(int an){
   ...
   int con=1;
   for(Articulo  x:obj.lisGenArt()){
       ds.setValue(x.getPrecio(), "ventas",x.getCoda());
       con++;
       if(con>an)break;
   }
  //graficar en memoria
  JFreeChart fg=ChartFactory.createBarChart3D("nro de articulos "+an, "coda","precio", ds,
  ...
  return fg;
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        response.setContentType("image/jpeg");
         ...
         switch(opc){
             case 1: ChartUtilities.writeChartAsJPEG(flujo, circulo(an), 650,530);break;
             case 2: ChartUtilities.writeChartAsJPEG(flujo, barra(an), 650, 530);break;
             case 3: ChartUtilities.writeChartAsJPEG(flujo, Lineal(an), 650, 530);break;
         }
         flujo.close();
